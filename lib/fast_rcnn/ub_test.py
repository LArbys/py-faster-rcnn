...
def test_net(net, imdb, max_per_image=100, thresh=0.05, vis=False):
    ...
    for i in xrange(num_images):
        ...
        image = imdb.image_path_at(i).split("/")[-1]
        ...
        for j in xrange(1, imdb.num_classes):
            ...
            #print "class: {} cls_scores: {}".format(j,np.sort(cls_scores)[::-1]) 
            cls_boxes = boxes[inds, j*4:(j+1)*4]
            cls_dets = np.hstack((cls_boxes, cls_scores[:, np.newaxis])) \
                         .astype(np.float32, copy=False)

            # apply non maximal supression, I found this
            # https://github.com/rbgirshick/rcnn/blob/master/nms/nms.m
    ...
    print 'Evaluating detections' #this is calling rpn uboone function!
